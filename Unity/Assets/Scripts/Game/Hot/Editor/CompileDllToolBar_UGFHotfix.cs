...
namespace Game.Hotfix.Editor
{
    internal sealed class CompileDllToolBar_UGFHotfix
    {
        ...
        private static readonly GUIContent s_ButtonGUIContent = new GUIContent($"Compile {s_DllName}", $"Compile {s_DllName} Dll.");
        ...
        static void OnToolbarGUI()
        {
            ...
            {
                BuildAssemblyHelper.Build(s_DllName, s_Codes, Array.Empty<string>(), Array.Empty<string>(), s_CodeOptimization);
                File.Copy(Path.Combine(BuildAssemblyHelper.BuildOutputDir, $"{s_DllName}.dll"), Path.Combine(s_CodeDir, $"{s_DllName}.dll.bytes"), true);
                File.Copy(Path.Combine(BuildAssemblyHelper.BuildOutputDir, $"{s_DllName}.pdb"), Path.Combine(s_CodeDir, $"{s_DllName}.pdb.bytes"), true);
