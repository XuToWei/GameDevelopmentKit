...
namespace ET.Server
{
	...
	public class C2G_EnterMapHandler : MessageSessionHandler<C2G_EnterMap, G2C_EnterMap>
	{
		protected override async UniTask Run(Session session, C2G_EnterMap request, G2C_EnterMap response)
		{
			...
			var startSceneConfig = Tables.Instance.DTStartSceneConfig.GetBySceneName(session.Zone(), "Map1");
			response.MyId = player.Id;

			// 等到一帧的最后面再传送，先让G2C_EnterMap返回，否则传送消息可能比G2C_EnterMap还早
			TransferHelper.TransferAtFrameFinish(unit, startSceneConfig.ActorId, startSceneConfig.Name).Forget();
		}
	}
}