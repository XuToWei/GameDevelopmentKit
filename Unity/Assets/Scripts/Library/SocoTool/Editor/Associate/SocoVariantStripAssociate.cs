...
using Soco.ShaderVariantsStripper;
using UnityEditor;
...
public class SocoVariantStripAssociate : Soco.ShaderVariantsCollection.IExecutable
{
    public override void Execute(ShaderVariantCollectionMapper mapper)
    {
        ...
        var stripConfigs = ShaderVariantsStripperCode.LoadConfigs();
        ...
        foreach (var shader in mapper.shaders)
        {
            ...
            foreach (var variant in variants)
            {
                ...
                if (ShaderVariantsStripperCode.IsVariantStrip(shader, snipperData, data, stripConfigs))
                {
