...
namespace Slate
{
    ...
    public class RenderWindow : EditorWindow
    {
        ...
        void OnGUI() {
            ...
            GUILayout.Space(20);
            EditorGUILayout.HelpBox("If you have already imported the package, please click the ENABLE button below.", MessageType.Info);
            if (!EditorApplication.isCompiling){
                if ( GUILayout.Button("ENABLE") ) { DefinesManager.SetDefineActiveForCurrentTargetGroup(Prefs.USE_FRAMECAPTURER_DEFINE, true); }
            } else {GUILayout.Button("Unity is compiling. Please wait...");}
            GUILayout.Space(20);
            EditorGUILayout.HelpBox("Please note that this rendering utility only supports the standard Unity render pipeline.", MessageType.Warning);
        }
