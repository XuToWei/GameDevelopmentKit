...
namespace NaughtyAttributes.Editor
{
	public static class NaughtyEditorGUI
	{
		...
		public static void Button(UnityEngine.Object target, MethodInfo methodInfo)
		{
			...
			var tp = methodInfo.GetParameters();
		...
		public static void NativeProperty_Layout(UnityEngine.Object target, PropertyInfo property)
		{
			...
			if (value == null)
			{
				...
				string warning = string.Format("{0} is null. {1} doesn't support reference types with null value", property.Name, "ShowNonSerializedFieldAttribute");
			...
			else if (!Field_Layout(value, property.Name))
			{
				...
				string warning = string.Format("{0} doesn't support {1} types", "ShowNonSerializedFieldAttribute", property.PropertyType.Name);
				HelpBox_Layout(warning, MessageType.Warning, context: target);
			}
		}
		...
		public static void NonSerializedField_Layout(UnityEngine.Object target, FieldInfo field)
		{
			...
			if (value == null)
			{
				...
				string warning = string.Format("{0} is null. {1} doesn't support reference types with null value", field.Name, "ShowNonSerializedFieldAttribute");
				HelpBox_Layout(warning, MessageType.Warning, context: target);
			}
			else if (!Field_Layout(value, field.Name))
			{
				...
				string warning = string.Format("{0} doesn't support {1} types", "ShowNonSerializedFieldAttribute");
				HelpBox_Layout(warning, MessageType.Warning, context: target);
			}
		}
