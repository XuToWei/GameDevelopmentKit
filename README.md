...
| GF 增强 | 支持同路径按类型加载 Sprite/Texture2D、WebSocket 网络通道与资源智能去冗余，自动分析共享依赖并合并零散资源包 |
| 统一异步 | 全项目使用 UniTask，替代 ETTask 并覆盖 GF 场景 |
| 响应式编程 | [ReactiveBinding] 以源码生成连接数据源与回调，支持字段、属性、方法、版本化集合、多源依赖自动推断与检查节流，减少手写监听和刷新逻辑 |
...
| 编辑器工具 | CodeBind、状态控制器、代码生成、Package 批量更新、Toolbar 与一键构建 |
