namespace ET
{
    ...
    public class AgentActorService : ActorBridgeService
    {
        ...
        private static int GetAgentFiberId(int agentProcessId)
        {
            ...
            return agents.First(a => a.Process == agentProcessId).Id;
        }
        ...
        internal const string DeployChunkPrefix = "__gdk_admin_chunk_";
        internal const string DeployFinalChunkMarker = "__gdk_admin_chunk_final";
        ...
        public async Task<bool> DeployFileAsync(int agentProcessId, string fileName, byte[] data, string targetPath)
        {
            ...
            // Chunked transfer: internal chunk markers append to a temporary file; an
            // explicit final marker prevents stale partial files from being mistaken
            // for the last chunk of a later small deployment.
            ...
            for (var i = 0; i < totalChunks; i++)
            {
                ...
                var request = Admin2Agent_DeployFileRequest.Create();
                request.FileName = isLast ? DeployFinalChunkMarker : $"{DeployChunkPrefix}{i}";
