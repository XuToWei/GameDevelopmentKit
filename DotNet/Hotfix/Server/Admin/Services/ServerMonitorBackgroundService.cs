...
namespace ET
{
    public class ServerMonitorBackgroundService : BackgroundService
    {
            ...
            ILogger<ServerMonitorBackgroundService> logger)
        {
            ...
            {
                throw new InvalidOperationException("Monitoring:PollingIntervalSeconds must be greater than zero");
            }
        ...
        {
            // BackgroundService.StartAsync runs synchronously until the first incomplete await.
            // Yield before the initial actor probes so unavailable game servers never delay the
            // web listener from coming online.
            await Task.Yield();
