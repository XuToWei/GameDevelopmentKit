...
namespace ET
{
    public class ProcessManagerService
    {
        ...
        public event Action OnProcessStatusChanged = delegate { };
        public bool MaintenanceMode { get; private set; }

        public ProcessManagerService(LogService logService, AdminActorService adminActorService,
            AgentActorService agentActorService, ConfigService configService,
            ILogger<ProcessManagerService> logger)
        {
            ...
            _logger = logger;
            MaintenanceMode = configService.MaintenanceEnabled;
        }
        ...
        {
            if (!await StopServerAsync(processId))
            {
                _logger.LogWarning("Restart aborted because process {ProcessId} could not be stopped", processId);
                return false;
            }
        ...
        public void SetMaintenanceMode(bool enabled)
        {
            ...
            _configService.MaintenanceEnabled = enabled;
            OnProcessStatusChanged();
        }
    }
}
